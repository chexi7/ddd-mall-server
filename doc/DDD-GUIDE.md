...
// 命令处理器（薄层，只做编排）—— 一个聚合一个 CommandHandler，方法名即命令类型
...
**优势**：业务规则集中在聚合根中，任何人改订单支付逻辑只需看 `Order.pay()`。`OrderCommandHandler.handle(PayOrderCommand)` 这种命名让调用方一眼看出——谁处理什么命令。
...
├─────────────────────────────────────┤
│      mall-application (应用层)        │  ← CommandHandler、QueryService、事件处理器
├─────────────────────────────────────┤
...
│       ├── command/                      # 写操作（按聚合扁平分包）
│       │   ├── product/
│       │   │   ├── ProductCommandHandler.java        #   handle(CreateProductCommand) / handle(PublishProductCommand) / handle(ChangePriceCommand)
│       │   │   ├── CreateProductCommand.java        #   命令入参（不可变）
│       │   │   ├── ChangePriceCommand.java
│       │   │   └── PublishProductCommand.java
│       │   ├── order/
│       │   │   ├── OrderCommandHandler.java          #   handle(CreateOrderCommand) / handle(PayOrderCommand) / handle(CancelOrderCommand)
│       │   │   ├── CreateOrderCommand.java          #   命令入参（不可变，含内嵌 OrderItemParam / ShippingAddressParam）
│       │   │   ├── PayOrderCommand.java
│       │   │   └── CancelOrderCommand.java
│       │   └── auth/
│       │       ├── AuthCommandHandler.java           #   handle(AdminLoginCommand) / handle(MemberLoginCommand)
...
└── mall-web/                             # 接口层
    └── web/
        ├── controller/
        │   ├── product/ProductController.java   #   仅注入 ProductCommandHandler + ProductQueryService
...
| **应用层按聚合扁平分包** | `command/order/`、`query/order/`、`eventhandler/order/`；同聚合的 `*CommandHandler` / `*Command` / `*Result` 平铺在一起，不再嵌套 `cmd/`、`handler/`、`result/` |
| **接口层按聚合分包** | `controller/order/`、`request/order/` |
| **基础设施层按技术关注点分包** | `dataobject/`、`converter/`、`impl/` |
| **所有 CommandHandler 方法必须接受 Command 对象** | 例如 `handle(PayOrderCommand command)` 而非 `payOrder(String orderNo)`，保持入参类型安全与可扩展性 |
...
- **一个聚合一个 `*CommandHandler`，方法名即命令类型**（对应 DDD 铁律：一个命令处理器只操作一个聚合）。例：`OrderCommandHandler` 暴露 `handle(CreateOrderCommand)` / `handle(PayOrderCommand)` / `handle(CancelOrderCommand)`，方法重载由 Command 类型自然区分，无需为每种命令各建一个 Handler 类。
- 命令入参使用 `*Command`（无业务含义的纯数据载体），有业务返回的用例使用 `*Result` 作为出参，与对应的 `*CommandHandler` 同包同层放置。
...
- `Handler` 后缀保留给领域事件处理器（`*EventHandler`）和命令处理器（`*CommandHandler`），不再用于其他场景。
...
1. **在扁平包里区分角色**：同一个 `command/admin/` 下放着 `RoleCommandHandler`（命令处理器）、`AssignPermissionsCommand`（入参），靠后缀一眼识别。
2. **名词 vs 动词不再混乱**：类 `AssignPermissionsCommand` 是名词（"分配权限"这次动作的具象），方法 `roleCommandHandler.handle(cmd)` 是动词（执行）。如果类名也叫 `AssignPermissions`，调用处变成 `handler.handle(new AssignPermissions(...))`，主谓全丢。
...
#### 4.4.2 CommandHandler 有响应时用 `*Result`，不要用 `*Response`
...
**`*Result` 命名规则**：用动词短语前缀，与对应 `*Command` 配对，与 `*CommandHandler` 同包平铺：
...
| 返回内容 | 做法 | 本项目对应例子 |
|---|---|---|
| `void`（命令成功即可） | 不建类 | `OrderCommandHandler.handle(PayOrderCommand)` / `handle(CancelOrderCommand)` |
| 单个原始类型（`Long` / `String` / `Boolean`） | 直接返回原始类型 | `MenuCommandHandler.handle(CreateMenuCommand) → Long`（菜单 ID）；`OrderCommandHandler.handle(CreateOrderCommand) → String`（订单号） |
| 多字段、有内聚业务含义 | 建 `*Result` 类 | `AuthCommandHandler.handle(AdminLoginCommand) → AdminLoginResult`（token + 用户 + 角色 + 权限） |
...
command/
├── order/
│   ├── OrderCommandHandler.java        ← 后缀识别角色
...
- 看到后缀 `*Command` → 知道是入参；看到 `*CommandHandler` → 知道是命令处理器
...
```
Controller → CommandHandler → new AggregateRoot(...) → Repository.save()
```

```java
// OrderCommandHandler.java —— 一个聚合一个命令处理器
@Transactional
...
```
Controller → CommandHandler → Repository.findById() → AggregateRoot.doSomething() → Repository.save()
```

```java
// OrderCommandHandler.java —— 同一个聚合的另一个命令处理方法
@Transactional
public void handle(PayOrderCommand command) {
...
| **聚合间通过事件通信** | 下单后库存预扣，用 OrderCreatedEvent | 不在 `OrderCommandHandler.handle(CreateOrderCommand)` 里直接调 `InventoryRepository` |
...
| **调用者** | CommandHandler（写） | QueryService（读） |
...
```java
// 标准三部曲：取出 → 调用 → 保存。由 CommandHandler 持有
@Transactional
...
**解决跨聚合协作问题**。下单后需要扣库存、清购物车——如果在一个 CommandHandler 里直接调用三个仓储，聚合之间就耦合了。

```
❌ 聚合间耦合方式：OrderCommandHandler.handle(CreateOrderCommand) 里直接调用 InventoryRepository + CartRepository
...
```
写（Command）：Controller → CommandHandler → AggregateRoot → Repository.save()
...
| `@RequiredArgsConstructor` | CommandHandler、DomainService、EventHandler | 构造器注入 |
...
**通过领域事件**。`Order` 创建后发布 `OrderCreatedEvent`，`OrderCreatedEventHandler` 监听并发出 `LockInventoryCommand`。不要在一个 CommandHandler 中同时操作多个聚合的仓储。
